...
void features_get_header(struct lucet_vmctx *ctx, guest_ptr_t key_ptr, guest_size_t key_len,
                         guest_ptr_t value_ptr, guest_ptr_t value_len_ptr)
{
    ...
    uint32_t *           val_len = (uint32_t *) &heap[value_len_ptr];
    if (lucet_vmctx_check_heap(ctx, (void *) key, key_len) &&
        lucet_vmctx_check_heap(ctx, (void *) val_len, sizeof(uint32_t)) &&
        lucet_vmctx_check_heap(ctx, (void *) val, *val_len)) {
        session_get_header(s, key, key_len, val, val_len);
    } else {
        lucet_vmctx_terminate(ctx, (void *) "features_get_header check_heap failed");
    }
}

// Morally, the signature of example_syscall_send is (const char* buf, size_t
// buf_len). The buf pointer has been translated into a heap offset by
// caller, and size_t is restricted to 32 bits to emulate wasm32.
void features_send(struct lucet_vmctx *ctx, guest_ptr_t buf_ptr, guest_size_t buf_len)
{
    char *          heap = lucet_vmctx_get_heap(ctx);
    struct session *s    = (struct session *) lucet_vmctx_get_delegate(ctx);
    // Syscall args are:
    ...
    if (lucet_vmctx_check_heap(ctx, (void *) buf, buf_len)) {
        ...
        lucet_vmctx_terminate(ctx, (void *) "features_send check_heap failed");
    }
}
