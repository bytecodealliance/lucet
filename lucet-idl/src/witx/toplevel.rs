...
pub fn parse_witx<P: AsRef<Path>>(i: P) -> Result<Vec<DeclSyntax>, WitxError> {
    ...
    let input_path = i_buf
        .canonicalize()
        .map_err(|e| WitxError::Io(i_buf.clone(), e))?;
    let input =
        fs::read_to_string(&input_path).map_err(|e| WitxError::Io(input_path.clone(), e))?;
    ...
    for t in toplevel {
        match t {
            ...
            TopLevelSyntax::Use(u) => {
                ...
                abs_path
                    .canonicalize()
                    .map_err(|e| WitxError::Io(u_path, e))?;
                ...
                if !used.contains(&abs_path) {
                    ...
                    let source_text = fs::read_to_string(&abs_path)
                        .map_err(|e| WitxError::Io(abs_path.clone(), e))?;
