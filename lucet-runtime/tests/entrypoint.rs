...
/// Called by the guest in `ctype`; must be outside the macro so it doesn't get duplicated.
#[no_mangle]
extern "C" fn black_box(_vmctx: *mut c_void, _val: *mut c_void) {}

const C_CALCULATOR_MOD_PATH: &'static str = "lucet-runtime-c/test/build/entrypoint/calculator.so";
const WAT_CALCULATOR_MOD_PATH: &'static str = "tests/build/entrypoint_guests/calculator.so";
const USE_ALLOCATOR_SANDBOX_PATH: &'static str = "tests/build/entrypoint_guests/use_allocator.so";
