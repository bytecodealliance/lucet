...
use crate::test_helpers::{guest_module_path, test_ex, test_nonex, DlModuleExt};
...
fn illegal_instr() {
    test_nonex(|| {
        ...
        assert!(state_str
            .starts_with("fault BadSignature triggered by Illegal instruction: code at address"));
...
fn hostcall_error() {
    test_nonex(|| {
        let module = DlModule::load_test(HOSTCALL_ERROR_SANDBOX_PATH).expect("module loads");
        let region = MmapRegion::create(1, &Limits::default()).expect("region can be created");
        let mut inst = region
            .new_instance(Box::new(module))
            .expect("instance can be created");
    ...
    test_ex(|| {
        ...
        match fork().expect("can fork") {
            ForkResult::Child => {
                let module = DlModule::load_test(TRAPS_SANDBOX_PATH).expect("module loads");
                let region =
                    MmapRegion::create(1, &Limits::default()).expect("region can be created");
                let mut inst = region
                    .new_instance(Box::new(module))
                    .expect("instance can be created");
...
fn sigsegv_handler_during_guest() {
    ...
    test_ex(|| {
        ...
        let saved_sa = unsafe { sigaction(Signal::SIGSEGV, &sa).expect("sigaction succeeds") };
