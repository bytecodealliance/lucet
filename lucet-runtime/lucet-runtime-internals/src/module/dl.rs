...
use lucet_module::{
    ...
    Signature, VersionInfo, LUCET_MODULE_SYM,
};
...
impl DlModule {
    ...
    #[cfg(feature = "signature_checking")]
    pub fn load_and_verify<P: AsRef<Path> + Copy>(
        so_path: P,
        pk: PublicKey,
        ...
        Self::load_and_maybe_verify(so_path, |module_data| {
            ...
            ModuleSignature::verify(so_path, &pk, &module_data).map_err(|e| e.into())
        })
    }

    fn load_and_maybe_verify<P: AsRef<Path>, F: FnOnce(&ModuleData) -> Result<(), Error>>(
        so_path: P,
        verifier: F,
    ) -> Result<Arc<Self>, Error> {
