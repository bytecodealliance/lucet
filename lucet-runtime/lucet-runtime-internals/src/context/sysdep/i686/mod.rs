...
pub(crate) struct GpRegs {
    ...
    pub(crate) edi: u32,
    pub(crate) esi: u32,
    pub(crate) fill: u32, // TODO: iirc this needs to be an even number of items?
}
...
impl<'a> CallStackBuilder<'a> {
    pub fn new(stack: &'a mut [u32]) -> Self {
    ...
    /// complete unit will be 8-byte aligned.
    ///
    /// TODO: dunno if this is necessary for any calling convention. just took 64-bit numbers and
    /// cut em in half.
    ///
    /// That is to say, `args` will be padded such that the current top of stack is 4-byte
    /// aligned.
    fn store_args(&mut self, args: &[u32]) {
    ...
    fn into_inner(self) -> (&'a mut [u32], usize) {
...
impl Context {
    ...
    ///     stack pointer by a variable amount, and it does not, so `esp` will continue to
    ///     point to guest arguments.
    ///   - This is why bootstrap recieves arguments via ebp, pointing elsewhere on the stack.
    ///
    ...
    pub fn init_with_callback(
        ...
        stack_builder.push(lucet_context_backstop as u32);
        stack_builder.push(fptr as u32);
        ...
        stack_builder.push(lucet_context_bootstrap as u32);
        ...
        child.gpr.esp = &mut stack[stack.len() - stack_start] as *mut u32 as u32;

        // Base pointer: `ebp` will be saved through all guest code, and preserved for when we
        // reach the backstop. This allows us to prepare an argument for `lucet_context_backstop`
        // even at the entrypoint of the guest.
        child.gpr.ebp = child as *const Context as u32;
    ...
    /// pointer is then replaced by the value saved in `to.gpr.esp`, so when `swap` returns, it will
    ...
    /// # Safety
    ///
    /// The value in `to.gpr.esp` must be a valid pointer into the stack that was originally passed
    ...
    /// is then replaced by the value saved in `to.gpr.esp`, so when `swap` returns, it will return
    ...
    /// result will be `0`.
    pub fn get_retval_gp(&self, idx: usize) -> u32 {
        self.retvals_gp[idx]
    }
...
/// TODO: is this necessary? is abi real?? does abi even align
/// Check whether the bottom (highest address) of the stack is 16-byte aligned, as required by the
/// ABI.
fn stack_is_aligned(stack: &[u32]) -> bool {
    let size = stack.len();
    let last_elt_addr = &stack[size - 1] as *const u32 as usize;
    let bottom_addr = last_elt_addr + mem::size_of::<u32>();
    bottom_addr % 8 == 0
}
