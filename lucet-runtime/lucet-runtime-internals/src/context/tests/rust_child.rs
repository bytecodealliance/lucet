...
#[test]
fn yield_nil() {
    unsafe extern "C" fn guest_fn() {
        Context::swap_yield(CHILD.as_mut().unwrap(), PARENT.as_ref().unwrap(), ());
    }

    test_body!(stack, {
        init_and_swap!(stack, guest_fn, []);
        unsafe {
            assert!(CHILD.as_ref().unwrap().yielded.is_some());
        }
    });
}
...
fn yield_5() {
    unsafe extern "C" fn guest_fn() {
        Context::swap_yield(CHILD.as_mut().unwrap(), PARENT.as_ref().unwrap(), 5u64);
    }

    test_body!(stack, {
        init_and_swap!(stack, guest_fn, []);
        unsafe {
            let yielded = CHILD.as_mut().unwrap().yielded.take().unwrap();
            let val = yielded.downcast::<u64>().unwrap();
            assert_eq!(*val, 5u64);
        }
    });
}
...
#[test]
fn yield_factorials() {
    unsafe extern "C" fn guest_fn(n: u64) -> u64 {
        if n <= 1 {
            Context::swap_yield(CHILD.as_mut().unwrap(), PARENT.as_ref().unwrap(), 1u64);
            1
        } else {
            let n = n * guest_fn(n - 1);
            Context::swap_yield(CHILD.as_mut().unwrap(), PARENT.as_ref().unwrap(), n);
            n
        }
    }

    test_body!(stack, {
        init_and_swap!(stack, guest_fn, [5u64.into()]);
        let mut facs = vec![];
        while let Some(yielded) = unsafe { CHILD.as_mut().unwrap().yielded.take() } {
            facs.push(*yielded.downcast::<u64>().unwrap());
            unsafe {
                Context::swap(PARENT.as_mut().unwrap(), CHILD.as_ref().unwrap());
            }
        }
        assert_eq!(facs.as_slice(), &[1, 2, 6, 24, 120]);
    });
}
...
fn coop_factorials() {
    unsafe extern "C" fn guest_fn(n: u64) -> u64 {
        if n <= 1 {
            ...
            let yielded = PARENT.as_mut().unwrap().yielded.take().unwrap();
            let n = *yielded.downcast::<u64>().unwrap();
    ...
    test_body!(stack, {
        ...
        let mut facs = vec![];
        while let Some(yielded) = unsafe { CHILD.as_mut().unwrap().yielded.take() } {
            if let Some(ns) = yielded.downcast_ref::<(u64, u64)>() {
                let (n, n_rec) = *ns;
                let n = n * n_rec;
                unsafe {
                    Context::swap_yield(PARENT.as_mut().unwrap(), CHILD.as_ref().unwrap(), n);
                }
            } else if let Some(n) = yielded.downcast_ref::<u64>() {
                facs.push(*n);
                unsafe {
                    Context::swap(PARENT.as_mut().unwrap(), CHILD.as_ref().unwrap());
                }
