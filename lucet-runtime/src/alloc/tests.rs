...
fn allocate_runtime_works() {
    let region = MmapRegion::create(1, &LIMITS).expect("region created");
    let mut inst = region
...
fn expand_past_spec_max() {
    let region = MmapRegion::create(10, &LIMITS).expect("region created");
    let mut inst = region
        .new_instance(MockModule::boxed_with_heap(&THREE_PAGE_MAX_HEAP))
        .expect("new_instance succeeds");
...
fn reject_initial_oversize_heap() {
    let region = MmapRegion::create(10, &LIMITS).expect("region created");
    let res = region.new_instance(MockModule::boxed_with_heap(&INITIAL_OVERSIZE_HEAP));
...
fn accept_small_guard_heap() {
    let region = MmapRegion::create(1, &LIMITS).expect("region created");
    let _inst = region
        .new_instance(MockModule::boxed_with_heap(&SMALL_GUARD_HEAP))
        .expect("new_instance succeeds");
}
...
fn reject_large_guard_heap() {
    let region = MmapRegion::create(1, &LIMITS).expect("region created");
    let res = region.new_instance(MockModule::boxed_with_heap(&LARGE_GUARD_HEAP));
    assert!(res.is_err(), "new_instance fails");
}
...
#[test]
fn reuse_slot_works() {
    fn peek_n_poke(region: &Arc<MmapRegion>) {
        let mut inst = region
            .new_instance(MockModule::boxed_with_heap(&ONE_PAGE_HEAP))
            .expect("new_instance succeeds");
...
fn context_alloc_child() {
    extern "C" fn heap_touching_child(heap: *mut u8) {
        let heap =
            unsafe { std::slice::from_raw_parts_mut(heap, CONTEXT_TEST_INITIAL_SIZE as usize) };
        heap[0] = 123;
        heap[4095] = 45;
    }

    let region = MmapRegion::create(1, &CONTEXT_TEST_LIMITS).expect("region created");
    let mut inst = region
        .new_instance(MockModule::boxed_with_heap(&CONTEXT_TEST_HEAP))
        .expect("new_instance succeeds");
...
fn context_stack_pattern() {
    ...
    extern "C" fn stack_pattern_child(heap: *mut u64) {
        let heap =
            unsafe { std::slice::from_raw_parts_mut(heap, CONTEXT_TEST_INITIAL_SIZE as usize / 8) };
    ...
    unsafe {
        ...
        assert!(stack_pattern + STACK_PATTERN_LENGTH < inst.alloc.slot().stack_top() as usize);
        let stack_pattern =
