...
TEST expect_stack_overflow_probestack(const char *path, int recursion_depth)
{
    ...
    const enum lucet_error err = lucet_instance_run(
        inst, "localpalooza", 1, (struct lucet_val[]){ LUCET_VAL_I32(recursion_depth) });
    ...
    //
    // TODO: the runtime no longer differentiates between different stack overflow sites after
    // moving the stack probe into lucetc; figure out a way to provide that information or just wait
    // till we can do the stack probe as a global symbol again
    //
...
SUITE(stack_suite)
{
    // The test with 64 locals should take up 252 bytes per stack frame. Along with the overhead for
    // the sandbox, that means it should overflow on the 455th recursion.  The trap table knows
    // about all of the instructions in the function that manipulate the stack, so the catch
    // mechanism for this is the usual one.
    ...
    // This test has about 1 page worth of locals - just enough for Cretonne to use probestack to
    // grow the stack. The 31st recursion should cause a stack overflow.
    ...
    // This test has 5000 locals - over 4 pages worth. Cretonne will use probestack here as
    // well. The 6th recursion should cause a stack overflow.
